...
class PerformanceMonitor {
  ...
  private getCurrentModel(): string {
    try {
      const config = execSync('cat config/active-model.json 2>/dev/null || echo "{}"', {
        encoding: "utf8",
      });
  ...
  private calculateTPS(): number {
    ...
    try {
      const output = execSync('ollama run phi3:mini "Count from 1 to 10" 2>&1', {
        encoding: "utf8",
        timeout: 10000,
      });
  ...
  private getGPUUsage(): number {
    try {
      ...
      const output = execSync('powermetrics --samplers gpu_power -i 1000 -n 1 2>&1 || echo "0"', {
        encoding: "utf8",
      });
  ...
  private getVRAMUsage(): number {
    try {
      const output = execSync(
        'nvidia-smi --query-gpu=memory.used --format=csv,noheader 2>&1 || echo "0"',
        { encoding: "utf8" },
      );
