...
class GPUOffloadManager {
  ...
  detectGPUs(): GPUInfo[] {
    ...
    try {
      // NVIDIA detection
      const nvidiaOutput = execSync('nvidia-smi --query-gpu=index,name,memory.total,memory.used,memory.free,utilization.gpu,temperature.gpu,cuda_cores --format=csv 2>/dev/null', { encoding: 'utf8' })
      ...
      try {
        const metalOutput = execSync('system_profiler SPDisplaysDataType 2>/dev/null | grep -A 20 "Metal"', { encoding: 'utf8' })
        this.gpus = [{
          id: 0,
  ...
  startMonitoring(intervalMs: number = 1000): void {
    ...
    setInterval(() => {
      ...
      this.gpus.forEach((gpu, idx) => {
        console.log(`   GPU ${idx}: ${gpu.utilization}% util, ${gpu.temperature}°C, ${gpu.vramUsed}/${gpu.vramTotal} MB`)
      })
      console.log()
    }, intervalMs)
  }
