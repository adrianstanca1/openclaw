...
function getSystemLoad(): { cpu: number; ram: number; vram: number } {
  try {
    const loadavg = execSync(
      'cat /proc/loadavg 2>/dev/null || sysctl -n vm.loadavg 2>/dev/null || echo "0.5"',
      { encoding: "utf8" },
    ).trim();
    const cpu = parseFloat(loadavg.split(" ").pop() || "0.5");

    const memInfo = execSync(
      'free -m 2>/dev/null || vm_stat 2>/dev/null || echo "MemTotal: 48000"',
      { encoding: "utf8" },
    );
...
function scoreModel(
  model: ModelMetrics,
  task: TaskProfile,
  load: { cpu: number; ram: number; vram: number },
): number {
  let score = 0;
