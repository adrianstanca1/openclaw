...
const HARDWARE_PROFILES: Record<string, BatchConfig> = {
  ...
  enthusiast: { parallelJobs: 4, batchSize: 128, maxContext: 8192, timeout: 90000, retryAttempts: 3 },
  workstation: { parallelJobs: 8, batchSize: 256, maxContext: 16384, timeout: 120000, retryAttempts: 3 },
}
...
function getHardwareProfile(): string {
  ...
  if (ramGB < 8) return 'minimal'
  if (ramGB < 16) return 'standard'
  if (ramGB < 32) return 'enthusiast'
  return 'workstation'
}
 ...
 */
async function executeBatch(prompts: string[], model: string, config: BatchConfig): Promise<BatchResult[]> {
...
async function executeSingle(
  ...
  try {
    ...
    return { jobId, status: 'failed', duration, tokens: 0, error: error.message }
  }
}
